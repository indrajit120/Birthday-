...
pattern = re.compile(
    r'<div>\s*<label className="block text-sm font-medium text-gray-700 mb-2">Character Photo</label>.*?</div>',
    re.DOTALL
)

replacement = """<div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">Character Photo</label>
                            {(characterPhotoPreview || (localConfig.endScene?.character?.image && not_emoji(localConfig.endScene?.character?.image) && len_gt_2(localConfig.endScene.character.image))) && (
...
# Wait, not_emoji and len_gt_2 are python logic, I should write the JS logic
...
if match:
    new_content = content[:match.start()] + replacement_js + content[match.end():]
